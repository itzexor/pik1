...
static void test_roundtrip(void) {
    ...
    CHECK(frame.header_len == sizeof(header));
...
static void test_empty_payload(void) {
    ...
    CHECK(pik_frame_encode(header, sizeof(header), NULL, 0,
                           dec, sizeof(dec), enc, sizeof(enc), &enc_len) == PIK_FRAME_OK);
    CHECK(pik_frame_decode(enc, enc_len, sizeof(enc), sizeof(header),
                           out, sizeof(out), &frame) == PIK_FRAME_OK);
    CHECK(frame.header_len == 1);
...
static void test_decode_oversized(void) {
    ...
    CHECK(pik_frame_decode(enc, enc_len, enc_len - 1, 2,
...
static void test_decode_short(void) {
    ...
    CHECK(pik_frame_decode(enc, dec_len, sizeof(enc), 1,
...
static void test_crc_mismatch(void) {
    ...
    CHECK(pik_frame_decode(enc, enc_len, sizeof(enc), 2,
...
static void test_cobs_bad_payload(void) {
    ...
    CHECK(pik_frame_decode(enc, sizeof(enc), sizeof(enc), 1,
