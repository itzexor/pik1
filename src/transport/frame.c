...
pik_frame_status_t pik_frame_encode(const uint8_t *header, size_t header_len,
                                    ...
                                    size_t *enc_len) {
    size_t body_len = header_len + payload_len;
    size_t dec_len = body_len + 4;
    if (dec_len > dec_cap)
        return PIK_FRAME_OVERSIZED;

    if (header_len)
        memcpy(dec, header, header_len);
    if (payload_len)
        memcpy(dec + header_len, payload, payload_len);
